...
/// <summary>
/// 单例，并通过委托传递信息给用户的提示面板数据处理的结果，数据库工具，全局调用，升级，修改密码
/// </summary>
...
break;
    ...
    //修改信息，修改密码为例
    public void selectpasswrod(string[] str)
    {
        string connectStr = "server=localhost;port=3306;database=mygamedb;user=root;password=scyscy111;";
        //定义数据库连接对象，传递对象
        MySqlConnection conn = new MySqlConnection(connectStr); //并没有建立数据库连接
...
//执行Mysql语句并读取结果
        MySqlCommand myCommand = new MySqlCommand("UPDATE users SET password= '"+str[1].ToString()+"' WHERE username='"+str[0].ToString()+"'", conn);
        Debug.Log("UPDATE users SET password= '"+str[1].ToString()+"' WHERE username='"+str[0].ToString()+"'");
        myCommand.ExecuteNonQuery();
      conn.Close();
            
    }
    //角色升级实时更新
    public void selectlever(string[] str)
    {
        string connectStr = "server=localhost;port=3306;database=mygamedb;user=root;password=scyscy111;";
        //定义数据库连接对象，传递对象
        MySqlConnection conn = new MySqlConnection(connectStr); //并没有建立数据库连接
...
//执行Mysql语句并读取结果
        MySqlCommand myCommand = new MySqlCommand("UPDATE users SET leve= '"+str[1].ToString()+"' WHERE username='"+str[0].ToString()+"'", conn);
        Debug.Log("UPDATE users SET password= '"+str[1].ToString()+"' WHERE username='"+str[0].ToString()+"'");
        myCommand.ExecuteNonQuery();
        conn.Close();
...
//执行Mysql语句并读取结果
        MySqlCommand myCommand = new MySqlCommand("delete from users where username='"+str.ToString()+"'", conn);
        Debug.Log("delete from users where username='"+str.ToString()+"'");
        myCommand.ExecuteNonQuery();
        conn.Close();
